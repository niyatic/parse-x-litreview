...
The contributions of this review are operational rather than methodological:

1. **A nine-axis coverage map** (seven measurement dimensions plus two stratification overlays) over the document-parsing literature 2019–2026, with explicit per-dimension prior-art catalogues.
2. **A unified model lineage table** (Appendix A) covering 50+ open-source and proprietary engines, with base architecture, training data, license, and version pin per row.
3. **A datasets-and-contamination posture register** (Appendix B) that names which corpora are public vs internal and flags known training-data overlap.
...
5. **An honest residuals list** (§4) naming reproducibility blockers, statistical-power limits, and four content surfaces (handwriting, math formula extraction, document classification, long-document handling) treated only briefly.
6. **A measurement-discipline template** ported from HELM [161] and BIG-bench [162], applied to document parsing for the first time as a standing release-gate rather than a per-paper choice.
 Running in parallel to that academic line is a sizeable industrial-IDP literature (UiPath Document Understanding [172], Hyperscience [175], Rossum [176], Microsoft 365 document processing [177], Google Document AI [174], AWS Textract [173], plus the Forrester/Gartner/Everest/IDC analyst taxonomies that segment them) which shares the same parse-then-extract architecture but reports customer-pilot headline accuracy rather than per-document vectors; §2.8 covers that landscape and its evaluation-discipline gap.
...
Four 2024-2025 surveys frame the literature that D-TABLE and the broader Parse-x dimensions sit inside, and naming them explicitly lets this review state its delta rather than re-derive their taxonomies.
...
Handwritten document recognition (Handwritten Text Recognition (HTR)) was named out-of-scope in v4 of this review, but the senior critique flagged that the printed-text framing leaves a measurable surface, scanned ledgers, signed forms, archival correspondence, uncovered. This subsection inventories the prior art so a future Parse-x overlay can be wired against it without re-discovery.
...
The discussion below is intentionally short; the substantive limitations of the framework, the reproducibility caveats, and the statistical-power floor are promoted to their own top-level Section 4 (Limitations) per NeurIPS-D&B convention.
...
- **Handwritten / cursive recognition** (formerly out-of-scope; brief coverage now in section 2.8). IAM database [178], READ / ICFHR competitions, and RIMES define the benchmark lineage; handwritten recognition retains its own failure modes and is not directly served by the printed-text engines surveyed here.
...
Two content surfaces are named as out-of-scope in §3.1 but warrant a brief survey because adjacent vendors routinely conflate them with general document parsing. This subsection summarizes prior art for each, names canonical benchmarks and modern engines, and states the explicit delta from the Parse-x primary parsing and extraction surface.
...
Document parsing sits upstream of two downstream surfaces with social consequences: business-process automation (invoice processing, contract intake, loan underwriting), where parse errors propagate to financial decisions; and digital archives / accessibility (scanning legal, medical, and historical records), where parse errors shape what is readable by downstream tools. The framework's per-document score vectors and apples-to-apples decoding pins are intended to surface systematic disparities, particularly across languages, document quality tiers, and document types that are over-represented in upstream training data. Two specific risks are noted. First, evaluation-set contamination: many of the surveyed VLMs were trained on web-scraped corpora that may overlap with the benchmarks they are evaluated on; Appendix B's contamination column flags known cases but cannot certify the closed-source frontier models. Second, evaluator-LLM bias (§4.4): when a downstream-task evaluator is itself an LLM (here, Qwen3.6-35B-A3B as the frozen extractor for D-DOWNSTREAM), the framework can rank engines reliably under the apples-to-apples constraint but cannot certify absolute capability. Users of the framework should report both the engine under test and the evaluator-LLM pin.
